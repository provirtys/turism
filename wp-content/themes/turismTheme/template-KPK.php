...
<body>
	<section class="main">
		<div class="main__container container">
			<h2>КПК РФ</h2>
			<?php global $wpdb;
			$result = $wpdb->get_results("select id_kpk, kpk.Name, Head_kpk,kpk.email,Start_date,End_date, concat(people.Last_name, ' ', people.First_name, ' ', people.Patronymic) as Chairman, type_of_school.Name as Lvl from kpk
			inner join people on kpk.Chairman = people.ID_people
			inner join type_of_school on kpk.Powers = type_of_school.ID_type_of_school
			union 
			select id_kpk, kpk.Name, Head_kpk,kpk.email,Start_date,End_date, Chairman, type_of_school.Name as Lvl from kpk
			inner join type_of_school on kpk.Powers = type_of_school.ID_type_of_school
			...
			<table class="table table-edit" id="info-table">
				...
				<tbody>
					...
					foreach ($result as $kpk) {
						$dateStart = date('d.m.Y', strtotime($kpk->Start_date));
						$dateEnd = date('d.m.Y', strtotime($kpk->End_date));
						$today = date('d.m.Y');
					?>
						<tr>
						<?php if($userID == '100-00'){ ?>
							<td>
								<button value="<?php echo $kpk->id_kpk ?>" class="editBtn tableBtn">
		...
		C6.716,0,0,6.716,0,15v300c0,8.284,6.716,15,15,15h300c8.284,0,15-6.716,15-15V165.001C330,156.716,323.284,150.001,315,150.001z" />
							...
							<?php } ?>
							<td><?php echo $kpk->id_kpk; ?></td>
							<td><?php echo $kpk->Name; ?></td>
							<td><?php echo $kpk->Head_kpk; ?></td>
							<td><?php echo $kpk->Lvl; ?></td>
							...
							<td <?php if (strtotime($today) > strtotime($dateEnd)) {
								...
								<?php
								echo $dateEnd; ?></td>
							<td><?php echo $kpk->Chairman; ?></td>
							<td><?php echo $kpk->email; ?></td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
	...
	<div class="modals modals-kpk">
		<div class="modal">
			<div class="main__container container">
				<form class="form">
					<p class="form__title">Изменить КПК</p>
					<div class="form__block">
						...
						if ($powers >= 4) { ?>
							<div class="form__row">
								...
								<select name="kpkHead" id="kpkHead">
									...
									foreach ($result as $kpk) { ?>
										<option value="<?php echo $kpk->ID_kpk ?>"><?php echo $kpk->ID_kpk . ' ' . $kpk->Name; ?></option>
									<?php } ?>
								</select>
							</div>
							<div class="form__row">
						...
						?>
						<div class="form__row">
							<label for="kpkEmail">Email</label>
							<input disabled id="kpkEmail" type="text" name="kpkEmail">
						</div>
