...
<body>
	<section class="main">
		<div class="main__container container container__people">
			...
			$positions = $wpdb->get_results("select * from positions where id_position in (11,14)");
			$result = $wpdb->get_results("select  people.*, concat(people.last_name, ' ' ,people.first_name, ' ', people.patronymic) as FIO, positions.Name as position
			from ips
			inner join people on ips.ID_people = people.ID_people
			inner join positions on positions.id_position = ips.id_position
			where ID_ips in(
			select max(id_ips)
				 from ips
			...
			<table <?php if ($result == null)
						echo 'hidden'; ?> class="table tableSearch table-delete" id="info-table">
				<thead>
					<tr>
						<th></th>
						<th onclick="sortTable(0)" value="Last_name" class="th">ФИО</th>
						<th onclick="sortTable(1)" value="Date_of_birth" class="th">Дата рождения</th>
						<th onclick="sortTable(1)" value="Position" class="th">Должность</th>
						<th onclick="sortTable(3)" value="Telephone" class="th">Телефон</th>
						<th onclick="sortTable(4)" value="Email" class="th">Email</th>
					</tr>
				</thead>
				<tbody>
					<?php
		...
		S6.913,17.49,12.202,12.202z" />
							...
							<td><?php
								if ($man->position == 'Начальник школы') {
									echo '<b>'.$man->position.'</b>';
								}
								else{
									echo $man->position;
								}
			...
			<select hidden class="addPosition" name="position" id="position">
				<option value="notChosen">-Выберите должность-</option>
				<?php
		...
		S6.913,17.49,12.202,12.202z" />
			...
			<p class="no-rows" hidden>Работников нет</p>
