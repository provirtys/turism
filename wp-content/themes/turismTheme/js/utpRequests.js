	...
	if (table.rows.length > 1) {
		...
		acceptBtns.forEach(acceptBtn => {
			acceptBtn.addEventListener("click", () => {
				idUtp = acceptBtn.value
				data = {
					action: "acceptUtpRequest_action",
					idUtp: idUtp,
...
submitBtn.addEventListener('click', () => {
	...
	let data = {
		action: "declineUtpRequest_action",
		idUtp: idUtp,
		reason: reason,
	}
...
function changeTable(response) {
	...
	response.forEach(el => {
		tableBodyString += '<tr> <td>' + el['name'] +
			...
			'</td> <td>' + el['leader'] +
			`<td>
				<button class="tableBtn acceptBtn" value="`+ el['ID_utp'] + `">
		...
		<td>
			<button class="tableBtn declineBtn" value="`+ el['ID_utp'] + `">
