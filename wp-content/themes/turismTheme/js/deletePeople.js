...
function startPage() {
	...
	deleteBtnElems.forEach((deleteBtn) => {
		deleteBtn.addEventListener("click", () => {
			...
			let id_people = deleteBtn.value;
			var data = {
				action: "deletePeople_action",
				id_people: id_people,
				type_people: document.querySelector('h2').textContent,
			}
			jQuery.post(
				myajaxurl.ajaxurl,
				data,
				function (result) {
					...
					<tr>
						<td>
							<button value="` + man['ID_people'] + `" class="deleteBtn tableBtn">
	...
	C6.716,0,0,6.716,0,15v300c0,8.284,6.716,15,15,15h300c8.284,0,15-6.716,15-15V165.001C330,156.716,323.284,150.001,315,150.001z" />
						...
						<td onclick="sortTable(0)">`+ man['Last_name'] + `</td>
						<td onclick="sortTable(1)">`+ man['First_name'] + `</td>
						<td onclick="sortTable(2)">`+ man['Patronymic'] + `</td>
						<td onclick="sortTable(3)">`+ man['Gender'] + `</td>
						<td onclick="sortTable(4)">`+ man['Date_of_birth'] + `</td>
						<td onclick="sortTable(5)">`+ man['Address'] + `</td>
						<td onclick="sortTable(6)">`+ man['Place_of_work'] + `</td>
						<td onclick="sortTable(7)">`+ man['Telephone'] + `</td>
						<td onclick="sortTable(8)">`+ man['Email'] + `</td>
					</tr>`;
					})
